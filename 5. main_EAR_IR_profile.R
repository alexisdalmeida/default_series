...
# point 72 / baliasni / tower research / cube research
...
for (bank_count in 1:length(bank_list)){
  ...
  price_curves<- get_disc_curve( basel_tenor=basel_tenor, full_tenor=full_tenor, disc=disc)
  basel_tenor_with_curves<- price_curves[[1]]
  full_tenor_with_curves<- price_curves[[2]]
  ...
  EAR_and_income_table<- rbind(income_profile, EAR_table)
  
  # Add the bank name
  Bank<- as.data.frame(rep(bank_name, nrow(EAR_and_income_table))); names(Bank)<- 'Bank'
  EAR_and_income_table<- cbind(Bank, EAR_and_income_table)
  rownames(EAR_and_income_table)<- NULL
  
  # Combined banks
  if(bank_count==1){
      final_table<- EAR_and_income_table
  }else{
      final_table<- rbind(final_table, EAR_and_income_table)
  }
  
  #=== BAR PLOT ==========
  #data<- EAR_and_income_table
  #data_sub<- data[ data$GL1 == 'Assets' & data$Scenario == 'Base', ]
...
write.table(EAR_out, "clipboard", sep="\t")

# Export to Excel
path3<- "~/R/IRRBB_Proj/output/"
file<- "EAR_Result_table_all_banks.csv"
