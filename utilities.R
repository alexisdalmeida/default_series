...
get_cf<- function(EAR_data){
    cf<- cbind( EAR_data[ ,'Scenario'],  
                EAR_data[ ,'Instrument_currency'], 
                EAR_data[ ,'GL_Level_1'],
                EAR_data[ , 9:ncol(EAR_data)] )
...
get_income_profile<- function( basel_tenor, cf, shock_table){      
    ...
    for (scen_count in 1:length(scen_vect)){   # =========== Scenario loop
      ...
      for (ccy_count in 1:length(ccy_vect)){   # =========== ccy loop 
        ...
        for (gl_count in 1:length(gl_vect)){   # =========== GL loop  
          ...
          cf_profile<- get_income(cf=cf, basel_tenor=basel_tenor_shocked, scen=scen, ccy=ccy, gl=gl  )
...
get_income<- function( cf, basel_tenor, scen, ccy, gl ){
  ...
  if( ccy %in% ccy_list ){ ccy_for_price= ccy }else{ccy_for_price = 'AED' }
  
  # get the parameters for the computation
  price_rate<- basel_tenor[  , ccy_for_price]
  price_time<- basel_tenor[  , 'Year']
  price_factor<- price_rate/100 * (1-price_time)
  
  # Compute CF and income
  N<-  cf_select[ , 4: ( ncol(cf_select)) ] # extract the CF
  original_cf<- apply(N, 2, sum) # sum the rows
  names(original_cf)<- basel_tenor[ ,'Basel_bucket' ] # update the name
  income_cf <- original_cf  * price_factor # compute inc
  table_out<- as.data.frame( rbind(original_cf, income_cf) )# collate
  
  original_cf * price_rate/100 * (1-price_time)
  
  # Sum CF
  Sum_CF<- c( sum(table_out[1,])  ,  sum(table_out[2,]) )
  # Labels
  Scenario<- c(scen, scen) ; Currency<- c(ccy, ccy) ; GL1<- c(gl, gl)
  CF_type <-  c('Original', 'Repriced' )
  #Combine
