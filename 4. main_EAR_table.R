...
for (bank_count in 1:length(bank_list)){
  ...
  # get DCF profiles
  dcf_profile<- get_dcf_profile(basel_tenor_with_curves, cf, shock_table )
  
  # get fx for the bank
  fx_table<- data.frame( fx_rate$FX, fx_rate[,bank_name]); names(fx_table)<- c('FX', bank_name)
  
  # === get starting unstressed EVE 
  source("code/compute_EVE_function.R")
  EVE_table<- compute_EVE(dcf_profile, fx_table )
  index_scen<- EVE_table$Scenario == 'Base'
  index_ccy<- EVE_table$Currency =='Aggregated'
  index_cf<- EVE_table$CF_type =='Discounted'
  index_gl<- EVE_table$GL1 =='EVE_bank_level'
  EVE_0<- EVE_table[index_scen & index_ccy & index_gl & index_cf, ]$Sum_CF[1]
  ...
  # ==== loop through shocks and recompute EVE each time
    ...
    for (j in 1: (   ncol(ir_shocks)-4 ) ){ # the shocks start at col 4
      ...
      index_cf<- EVE_table$CF_type =='Discounted'
      index_gl<- EVE_table$GL1 =='EVE_bank_level'
      EVE_stress<- EVE_table[index_scen & index_ccy & index_gl & index_cf, ]$Sum_CF[1]

      dEVE_abs<- EVE_stress - EVE_0
      dEVE_rel<- dEVE_abs / abs(EVE_0)
      
      # build a table
      if( j==1){ EVE_level<- EVE_stress }else{ EVE_level<-  c( EVE_level, EVE_stress) }
      if( j==1){ dEVE_abs_table<- dEVE_abs }else{ dEVE_abs_table<-  c( dEVE_abs_table, dEVE_abs) }
      if( j==1){ dEVE_rel_table<- dEVE_rel }else{ dEVE_rel_table<-  c( dEVE_rel_table, dEVE_rel) }
      
      
    }
  
  shock_vect<- as.numeric( ir_shocks[1,5:ncol(ir_shocks)] )
  plot( shock_vect, dEVE_abs_table/1000000  , type = 'l')
  plot( shock_vect, dEVE_rel_table           , type = 'l')
  
  # combine EVE and dEVE in a single table
  combined<- data.frame( shock_vect, EVE_level, dEVE_abs_table, dEVE_rel_table  )
  # add labels
  h1<- paste(bank_name,'EVE', sep="_" )
  h2<- paste(bank_name,'dEVE_abs', sep="_" )
  h3<- paste(bank_name,'dEVE_rel', sep="_" )
...
plot(shock_vect/100, dEVE[,1], type='l', col=1,
      ylim = c(min_val,max_val), main = "Relative EVE change per bank", 
      xlab = "% IR shock", ylab = "EVE % change")
...
file<- "EVE_IR_profile_all_banks.csv"
